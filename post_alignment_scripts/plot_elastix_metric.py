...
for filename in sys.argv[1:]:
    ...
    #TODO validate the header indicates that the selected column
    # contains the thing we think it will contain
    ...
    plt.plot(iterations_with_metric_recorded,
             metric_values,
             '-',
             alpha=0.5,
             label=filename.split('elastix_Bspline/')[-1]) #label=np.std(metric_values))
